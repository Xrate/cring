...
static Point fitField(const NearestPointsField& np)
{
    double sx = 1 - np.relative.X, sX = np.relative.X;
    Point c00 = np.xyz * sx + np.Xyz * sX;
    Point c01 = np.xyZ * sx + np.XyZ * sX;
    Point c10 = np.xYz * sx + np.XYz * sX;
    Point c11 = np.xYZ * sx + np.XYZ * sX;

    double sy = (1 - np.relative.Y), sY = np.relative.Y;
    Point c0 = c00 * sy + c10 * sY;
    Point c1 = c01 * sy + c11 * sY;
...
Point FieldMapData::getFieldInPoint(const Point& p) const
{
	try
	{
